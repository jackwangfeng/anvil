...
- **类型**：`int`、`char`、`long`（64 位，含 `long long` / `long int`）、`short`、`unsigned` / `signed`（当前不区分符号性，按对应有符号类型处理）、`double`（`float` 视作 `double`）、指针 `T*`（多级）、数组 `T[N]`、`struct` / `union` / `enum` / `typedef`、`void`
...
- 结构体数组与结构体指针算术（元素 size 非 2 的幂时下标缩放退化）、多维数组、位域
