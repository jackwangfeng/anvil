...
> 覆盖标量/指针/数组/结构体字段访问/控制流/调用(含 `printf` 变参);
> **暂不支持**结构体按值传参/返回、函数指针、用户可变参数(用这些时改用原生后端)。
> 集成测试里有「原生后端 vs LLVM 后端结果对拍」确保两条路一致。
...
- **类型**：`int`、`char`、`long`（64 位，含 `long long` / `long int`）、`short`、`unsigned` / `signed`（当前不区分符号性，按对应有符号类型处理）、`double`（`float` 视作 `double`）、指针 `T*`（多级）、数组 `T[N]`（含多维 `T[M][N]`）、**函数指针** `RET (*f)(...)`、`struct` / `union` / `enum`（可作类型）/ `typedef`、`void`
...
- **初始化**：聚合初始化列表 `int a[3]={1,2,3}`、`int a[]={...}`（推断长度）、`struct P p={1,2}`，不足部分零填充（`{0}` 惯用法清零）
...
- `unsigned` 的**无符号语义**(现在 `unsigned`/`short` 仅作语法接受,按有符号 int/long 处理)、`long double`
...
- `char` 局部变量不截断到 8 位；`int` 按 32 位、`long` 按 64 位处理
