...
- **libc 互操作**：函数原型驱动正确的返回宽度与可变参数；`printf`（可变参数走栈）、`malloc`（指针返回）、`fprintf(stderr,…)`（`extern` 全局）等可直接用；内置 `<stdio.h>` `<stdlib.h>` `<string.h>` `<ctype.h>` `<stddef.h>` `<stdbool.h>` `<limits.h>` `<stdint.h>` `<math.h>` `<assert.h>` 等系统头（含 `size_t`/`NULL`/`bool`/`INT_MAX` 等 typedef 与宏，过预处理并带 `#ifndef` 守卫）
...
| **M25** | 扩展内置 libc 系统头(过预处理,支持 `#define`/守卫)、`extern` 全局(`stdin`/`stdout`/`stderr` 链接 libc)、内部 panic 降级为干净 `error:` 诊断 |
